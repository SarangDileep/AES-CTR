...
unsigned long int H[] = {0x6a09e667f3bcc908, 0xbb67ae8584caa73b,
...
inline static unsigned long int Ch(unsigned long int x, unsigned long int y,
                                   unsigned long int z) {
  return ((x & y) ^ (~x & z));
}
inline static unsigned long int Maj(unsigned long int x, unsigned long int y,
                                    unsigned long int z) {
  return ((x & y) ^ (x & z) ^ (y & z));
}
...
void hashcomputation(char *input) {
  ...
  binary = (unsigned int *)calloc(
      length + k + 1 + 16,
      sizeof(int)); // [message in ascii]+[1 bit]+[k 0 padding] + [message
                    // length in bits - big endian last 128 bits]
  ...
  for (int chunk = 0; chunk < (strlen(input) + 1 + 16 + k) / 128;
       chunk++) { // CHUNK ITERATION

    int j = 0 + (chunk * 128);

    for (int i = 0; i < 16; i++) {
      W[i] = (unsigned long)binary[j + 0] << 56 | (unsigned long)binary[j + 1]
                                                      << 48;
      W[i] += (unsigned long)binary[j + 2] << 40 | (unsigned long)binary[j + 3]
                                                       << 32;
    ...
    for (int t = 16; t < 80; t++) {
      W[t] =
          lowerSigma1(W[t - 2]) + W[t - 7] + lowerSigma0(W[t - 15]) + W[t - 16];
    }

    unsigned long int a = H[0], b = H[1], c = H[2], d = H[3], e = H[4],
                      f = H[5], g = H[6], h = H[7];
