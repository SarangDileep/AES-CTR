...
/* **************************KEY_SCHEDULING********************************** */
//This is the part where we take the initial 16 byte key and make it long enough for 10 rounds of AES

void keySchedule(uint8_t* key, int round){
    uint8_t first = key[3];
    uint8_t arr[4] = {key[3], key[7], key[11], key[15]}; //Making an array of all the key elements coming in the last col
    for(int i=0;i<3;i++){
        arr[i] = sBox[arr[i+1]]; //Circular shift + sbox substitution in aline
    }
    arr[3] = sBox[first];
    for(int i=0;i<4;i++){
        key[i*4] = key[i*4] ^ arr[i] ^ rcon[round][i]; //Preparing the elements that are multiples of 4
    }
    for(int i=1;i<4;i++){
        key[i] = key[i] ^ key[i-1];         
...
void keyExpansion(uint8_t* key){
    ...
    for(int i=0;i<10;i++){                    //Calls the key schedule function for each round basically helps selecting rcon values
    keySchedule(k, i);
        for(int j=0;j<16;j++){
            expandedKey[i][j] = k[j];
        }
