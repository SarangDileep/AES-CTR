...
inline static bool FBT(unsigned long int number,
                       int position) { // find bit value
...
inline static uint32_t combineTo32(uint8_t *seed) {
  return ((uint32_t)seed[3] << 24) | ((uint32_t)seed[2] << 16) |
         ((uint32_t)seed[1] << 8) | ((uint32_t)seed[0] << 0);
}
 ...
 *The newly generated seed becomes the seed for next round and the above
 *procedure repeats In the end we have a stream of 1's and 0's as output
 */
void LFSR(uint8_t *seed) {

  bool gateBit[4], outputBit[4];
  uint8_t containerSize = 0;
  uint32_t _seed[4] = {combineTo32(seed), combineTo32(&seed[4]),
                       combineTo32(&seed[8]), combineTo32(&seed[12])};
  pvector polynomials[4]; // Vectors containing the tap positions of the LFSR
  uint64_t container = 0;

  for (int i = 0; i < 4; i++)
    populatePolynomial(&polynomials[i],
                       _seed[i]); // Creating the polynomial of the integer
